...
class Yang extends TagLib {
	...
	protected $tags = array(
		...
		//文章列表
		'artlist' => array(
			'attr' => 'flag,typeid,arcid,titlelen,infolen,pointstart,pointend,auditflag,orderby,keyword,fieldflag,limit,pagesize,pageroll,pagetheme', //attr 属性列表,arcid[new|20140413] 指定文档ID
			'close' => 1, // close 是否闭合（0 或者1 默认为1，表示闭合）
		),
		//产品列表分页
		'prolist' => array(
			'attr' => 'flag,typeid,arcid,titlelen,infolen,pointstart,pointend,auditflag,orderby,keyword,fieldflag,limit,pagesize,pageroll,pagetheme',
			'close' => 1,
		),
		//图片列表分页
		'piclist' => array(
		...
		//通用列表
		'list' => array(
			'attr' => 'flag,typeid,titlelen,infolen,pointstart,pointend,auditflag,orderby,keyword,fieldflag,limit,pagesize,pageroll,pagetheme',
			'close' => 1,
		),

		//全局搜索文档列表--20171018
		'slist' => array(
			'attr' => 'typeid,modelid,arcid,titlelen,infolen,pointstart,pointend,auditflag,orderby,keyword,limit,pagesize,pageroll,pagetheme',
			'close' => 1,
		),
	...
	public function _artlist($attr, $content) {
		...
		$auditflag = empty($attr['auditflag']) ? 0 : intval($attr['auditflag']); //0(已审核),1未审核,2全部，
...
<?php
	...
	//0(已审核),1未审核,2全部，
	if($auditflag == 1) {
		\$where['article.audit_status'] = 0;
	} else if($auditflag == 2) {
	} else {
		\$where['article.audit_status'] = 1;
	}
...
<?php
	...
	//0(已审核),1未审核,2全部，
	if($auditflag == 1) {
		\$where['product.audit_status'] = 0;
	} else if($auditflag == 2) {
	} else {
		\$where['product.audit_status'] = 1;
	}
...
<?php
	...
	//0(已审核),1未审核,2全部，
	if($auditflag == 1) {
		\$where['picture.audit_status'] = 0;
	} else if($auditflag == 2) {
	} else {
		\$where['picture.audit_status'] = 1;
	}
...
<?php
	...
	//0(已审核),1未审核,2全部，
	if($auditflag == 1) {
		\$where['soft.audit_status'] = 0;
	} else if($auditflag == 2) {
	} else {
		\$where['soft.audit_status'] = 1;
	}
...
str;
	...
	public function _reviewlist($attr, $content) {
		...
		$orderby = empty($attr['orderby']) ? 'id DESC' : $attr['orderby'];
		$limit = empty($attr['limit']) ? '10' : $attr['limit'];
		$type = empty($attr['type']) ? 0 : intval($attr['type']);
		$showflag = empty($attr['showflag']) ? 0 : intval($attr['showflag']);
